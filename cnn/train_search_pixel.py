...
parser.add_argument('--batch_increase', default=8, type=int,
...
def main():
    ...
    model = Network(args.init_channels, CIFAR_CLASSES, args.layers, criterion).to(device)  # 网络
    logging.info("param size = %fMB", utils.count_parameters_in_MB(model))

    optimizer = torch.optim.SGD(  # 随机梯度下降算法用于优化模型权重
        model.parameters(),
        args.learning_rate,
        momentum=args.momentum,
        weight_decay=args.weight_decay)

    # train_data, test_data = load_data(image_file, label_file)
    data = np.load('./indian_10p_pixel.npz')
    ...
    post_train = 5
    epochs = args.warmup_dec_epoch + args.decision_freq * (num_edges - 1) + post_train + 1  # 迭代次数 9 5 4 5 1 24
    ...
    architect = Architect(model, args)  # 架构

    normal_selected_idxs = torch.tensor(len(model.alphas_normal) * [-1], requires_grad=False, dtype=torch.int).to(
        device)
    reduce_selected_idxs = torch.tensor(len(model.alphas_reduce) * [-1], requires_grad=False, dtype=torch.int).to(
        device)
    normal_candidate_flags = torch.tensor(len(model.alphas_normal) * [True], requires_grad=False,
                                          dtype=torch.bool).to(device)
    reduce_candidate_flags = torch.tensor(len(model.alphas_reduce) * [True], requires_grad=False,
                                          dtype=torch.bool).to(device)
