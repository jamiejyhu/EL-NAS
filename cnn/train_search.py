...
parser.add_argument('--batch_increase', default=16, type=int,
...
parser.add_argument('--steps', type=int, default=5, help='1 2 3 4 5 6 7 8')
parser.add_argument('--model_path', type=str, default='saved_models', help='path to save the model')
parser.add_argument('--save', type=str, default='EXP', help='experiment name')
parser.add_argument('--seed', type=int, default=0, help='random seed')

parser.add_argument('--dataset', type=str, default='./datasets/pavia_1p.npz', help='indian pavia houston salinas')
parser.add_argument('--category', type=int, default='9', help='Indian:16 PaviaU:9 Salinas:16 Houston:15 imdb:18')
parser.add_argument('--datachannel', type=int, default='103',
                    help='indian:200, pavia:103 176 salinas:204 houston:144 imdb:128')

parser.add_argument('--grad_clip', type=float, default=1, help='gradient clipping')
parser.add_argument('--train_portion', type=float, default=0.5, help='portion of training data')  # 0.3 0.4 0.5 0.6 0.7
parser.add_argument('--unrolled', action='store_true', default=False, help='use one-step unrolled validation loss')
parser.add_argument('--arch_learning_rate', type=float, default=3e-4, help='learning rate for arch encoding')
parser.add_argument('--arch_weight_decay', type=float, default=1e-3, help='weight decay for arch encoding')
parser.add_argument('--warmup_dec_epoch', type=int, default=5, help='warmup decision epoch')
parser.add_argument('--decision_freq', type=int, default=5, help='decision freq epoch')  # 决策频率
parser.add_argument('--use_history', default=True, help='use history for decision')
...
class DataSet(Dataset):
    def __init__(self, x, y):
        self.x_list = torch.tensor(x, dtype=torch.float)
        self.y_list = torch.tensor(y, dtype=torch.long)
...
def main():
    ...
    ##########################################################################################
    CIFAR_CLASSES = args.category
    model = Network(args.init_channels, CIFAR_CLASSES, args.datachannel, args.layers, criterion, steps=args.steps).to(
        device)  # 网络
    ##########################################################################################
    logging.info("param size = %fMB", utils.count_parameters_in_MB(model))

    optimizer = torch.optim.SGD(  # 随机梯度下降
        model.parameters(),
        args.learning_rate,
    ...
    # data = np.load('./datasets/pavia_20.npz')  # 103
    # x_train, y_train = data['x_train'], data['y_train']
    # xxxx = np.zeros((x_train.shape[0], 204, x_train.shape[2], x_train.shape[3]))
    # xxxx[:, :x_train.shape[1], :, :] = x_train
    # print(x_train.shape)
    # data = np.load('./datasets/salinas_20.npz')  # 204
    # print(data['x_train'].shape)
    # x_train = np.append(xxxx, data['x_train'], axis=0)
    # y_train = np.append(y_train, data['y_train'] + max(y_train))
    # print(x_train.shape)
    # data = np.load('./datasets/houston_20.npz')  # 144
    # print(data['x_train'].shape)
    # xxxx = np.zeros((data['x_train'].shape[0], 204, data['x_train'].shape[2], data['x_train'].shape[3]))
    # xxxx[:, :data['x_train'].shape[1], :, :] = data['x_train']
    # x_train = np.append(x_train, xxxx, axis=0)
    # y_train = np.append(y_train, data['y_train'] + max(y_train))
    # print(x_train.shape)
    # print('y_train', y_train)
    ####################################################################################

    num_train = len(x_train)  # 训练数据量
    print('数据量', num_train)
    indices = list(range(num_train))  # 训练数据索引
    split = int(np.floor(args.train_portion * num_train))  # 1/2

    train_data = DataSet(x_train, y_train)
    train_queue = torch.utils.data.DataLoader(  # 训练数据分批
        train_data, batch_size=args.batch_size,
        sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[:split]),
        pin_memory=True)

    valid_queue = torch.utils.data.DataLoader(  # 验证集
        train_data, batch_size=args.batch_size,
        sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[split:num_train]),
        pin_memory=True)
    # train_queue = data_iter(args.batch_size, x_train, y_train)
    # valid_queue = data_iter(args.valid_batch_size, x_valid, y_valid)

    num_edges = model._steps * 2  # 8
    post_train = 5  # 9                  5                7               5
    epochs = args.warmup_dec_epoch + args.decision_freq * (num_edges - 1) + post_train + 1  # 27
    logging.info("total epochs: %d", epochs)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(  # 余弦退火 学习率有序调整
        optimizer, float(epochs), eta_min=args.learning_rate_min)
    ####################################################
    architect = Architect(model, args)  # 架构类
    ####################################################
    normal_selected_idxs = torch.tensor(len(model.alphas_normal) * [-1], requires_grad=False, dtype=torch.int).to(
        device)
    normal_candidate_flags = torch.tensor(len(model.alphas_normal) * [True], requires_grad=False,
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        input_search, target_search = next(iter(valid_queue))  # 取出一批量验证集数据
        ...
        loss.backward()
        nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)  # 梯度裁剪 防止梯度爆炸
        optimizer.step()
