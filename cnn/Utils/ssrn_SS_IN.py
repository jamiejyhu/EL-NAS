    ...
    def f(input):
        for i in range(repetitions):
            ...
            input = block_function(
                ...
                is_first_block_of_first_layer=(is_first_layer and i == 0)  # 第一个块
            )(input)
        return input
...
def bottleneck(nb_filter, init_subsample=(1, 1, 1), is_first_block_of_first_layer=False):
    ...
    def f(input):
        ...
        if is_first_block_of_first_layer:
            ...
            conv_1_1 = _bn_relu_conv(nb_filter=nb_filter, kernel_dim1=1, kernel_dim2=1, kernel_dim3=1,
                                     subsample=init_subsample)(input)
...
class ResnetBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn_spc, block_fn, repetitions1, repetitions2):
        ...
        Args:
            (1,7,7,200) ### input_shape: The input shape in the form (nb_channels, nb_rows, nb_cols)
            (16,) ### num_outputs: The number of outputs at final softmax layer
            "basic_block" ### block_fn: The block function to use. This is either `basic_block` or `bottleneck`.
                The original paper used basic_block for layers < 50
            [1],[1] ### repetitions: Number of repetitions of various block units.
        ...
        print("input shape:", input._keras_shape[3]) # 200

        # 3D卷积 高、宽、通道数、深度                                                                   步长 通道/2
